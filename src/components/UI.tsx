...
import { LucideIcon, Menu, Map, Users, Coins, Camera, ArrowLeft } from 'lucide-react';
...
export const MainMenu: React.FC<{ onStart: (mode: any) => void, user: UserData | null }> = ({ onStart, user }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-mc-bg p-6 bg-[url('https://picsum.photos/800/1200?blur=5')] bg-cover bg-center blend-overlay">
            <div className="bg-black/70 p-8 rounded-xl w-full max-w-md backdrop-blur-md border-2 border-white/20">
                ...
                <button onClick={() => onStart('world')} className={`${btnClass} bg-mc-green border-green-800`}>Open World</button>
