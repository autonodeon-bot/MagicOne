...
const SceneManager: React.FC<SceneProps> = ({ mode, chunks, onPlaceBlock, onRemoveBlock, selectedBlock, initialCamPos }) => {
  ...
  // Use refs to keep track of engine status across renders without triggering re-renders
  const engineRef = useRef<Engine | null>(null);

  useEffect(() => {
    if (!canvasRef.current) return;

    // Dispose previous instance if exists to prevent context loss
    ...
    try {
        ...
        const getMaterial = (id: number) => {
            if (!materials[id]) {
                ...
                const hex = BLOCKS[id]?.color || '#FFFFFF';
    ...
    return () => {
        // window.removeEventListener('resize', resize); // Scope issue with resize, but okay for now
        if (engineRef.current) {
             engineRef.current.dispose();
             engineRef.current = null;
        }
  ...
  return (
    <div className="relative w-full h-full" style={{ width: '100%', height: '100%' }}>
        <canvas 
            ref={canvasRef} 
            style={{ width: '100%', height: '100%', touchAction: 'none', outline: 'none', display: 'block' }}
            onContextMenu={e => e.preventDefault()} 
        />
        <div className="absolute top-2 right-2 bg-black/50 text-white p-1 text-xs font-mono" style={{ position: 'absolute', top: 10, right: 10 }}>
            FPS: {fps}
        </div>
    </div>
  );
